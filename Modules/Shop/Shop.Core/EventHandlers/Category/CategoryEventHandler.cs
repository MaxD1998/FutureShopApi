...
namespace Shop.Core.EventHandlers.Category;
...
public class CategoryEventHandler(IMediator mediator) : IMessageEventHandler
{
    ...
    public async Task ExecuteAsync(string message, CancellationToken cancellationToken)
    {
        ...
        switch (eventMessage.Type)
        {
            ...
            {
                var categoryEvent = JsonSerializer.Deserialize<EventMessageDto<CategoryEntity>>(message);
                if (categoryEvent?.Message != null)
                    await _mediator.Send(new CreateOrUpdateCategoryEntityCommand(categoryEvent.Message), cancellationToken);
