...
using Shop.Core.Cqrs.ProductBase.Command;
...
public class ProductBaseEventHandler(IMediator mediator) : IMessageEventHandler
{
    ...
    public async Task ExecuteAsync(string message, CancellationToken cancellationToken)
    {
        ...
        switch (eventMessage.Type)
        {
            ...
            {
                var productBaseEvent = JsonSerializer.Deserialize<EventMessageDto<ProductBaseEntity>>(message);
                if (productBaseEvent?.Message != null)
                    await _mediator.Send(new CreateOrUpdateProductBaseEntityCommand(productBaseEvent.Message), cancellationToken);
